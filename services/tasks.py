...
def taking_tasks(user_id):
    ...
    tasks_for_today = db_session.query(Tasks).filter(Tasks.end_date == today_date, Tasks.users.like(f"%{str(user_id)},%")).all()
    
    tom_date = taking_dates()[0][1]  
    tasks_for_tom = db_session.query(Tasks).filter(Tasks.end_date == tom_date, Tasks.users.like(f"%{str(user_id)},%")).all()
    
    tasks_for_other = db_session.query(Tasks).filter(Tasks.end_date != tom_date, Tasks.end_date != today_date, Tasks.users.like(f"%{str(user_id)},%")).all()
